...
export type ErrorStatusCode = StringToNumber<
  Extract<`${StatusCodes[keyof StatusCodes]}`, `4${number}` | `5${number}`>
>;
export type SuccessStatusCode = StringToNumber<
  Extract<`${StatusCodes[keyof StatusCodes]}`, `2${number}`>
>;
