...
describe('PATCH /v1/products/:id', () => {
  test('updates a product owned by the caller', async () => {
  ...
  test('returns 404 when the product belongs to another account', async () => {
    ...
    // Ensure Alice's product was not mutated.
    const aliceView = await app.request(`/v1/products/${aliceProduct.id}`, {
      headers: { Authorization: `Bearer ${alice.token}` }
    });
    const aliceBody = (await aliceView.json()) as { name: string };
    expect(aliceBody.name).toBe('Apple');
  });
